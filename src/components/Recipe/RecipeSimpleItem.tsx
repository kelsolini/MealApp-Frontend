...
const RecipeSimpleItem = ({ recipe }: RecipeItemProps) => {
    return (
        <Link to={`/oppskrifter/${recipe.id}`} className="block group max-w-sm w-full mx-auto">
            <article className="bg-sand/40 rounded-2xl overflow-hidden shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
                {/* Bilde */}
                ...
                {/* Innhold */}
                <div className="p-4">
                    <h3 className="text-ink text-xl font-serif font-semibold line-clamp-2 min-h-[3.5rem]">
                        {recipe.title}
                    </h3>
                </div>
            </article>
